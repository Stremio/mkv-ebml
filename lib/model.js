...
var nameToIdLookup = {};
var definitionKeys = Object.keys(ebmlIdDescriptions);

function processDefinitionEntry(idKeyString) {
    var definitionData = ebmlIdDescriptions[idKeyString];
    if (definitionData && typeof definitionData.name === 'string') {
        var nameKey = definitionData.name.replace(/-/g, '_');
        var numericIdValue = parseInt(idKeyString, 10);
        nameToIdLookup[nameKey] = numericIdValue;
    }
}

definitionKeys.forEach(processDefinitionEntry);

var exportedModel = {
	byEbmlID: ebmlIdDescriptions,
	byName: nameToIdLookup
};
